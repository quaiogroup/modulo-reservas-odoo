...
class OfficeBooking(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name') or vals['name'] in ('/', 'Nueva reserva', False):
                vals['name'] = (
                    self.env['ir.sequence'].next_by_code('office.booking')
                    or '/'
                )
    ...
    def action_mark_paid(self, tx_id=None):
        ...
        for rec in self:
            ...
            if not already_confirmed:
                rec._notify_customer("office_booking.mail_template_booking_confirmed_bold")
                rec._notify_admin("office_booking.mail_template_booking_confirmed_admin")
    ...
    def get_analytics_data(self):
        ...
        offices = self.env['office.space'].search([('active','=',True)])
    ...
    def _cron_cancel_expired_bookings(self):
        ...
        for booking in to_cancel:
            ...
            booking._notify_customer(
                "office_booking.mail_template_booking_cancelled_user"
            )
    ...
    def _cron_send_booking_reminders(self):
        ...
        for booking in bookings:
            booking._notify_customer("office_booking.mail_template_booking_reminder")
    ...
    def get_availability(self, office_id, date_str, exclude_id=None):
        ...
        blocked, reason = self.env["office.block"].is_date_blocked(office_id, date)
        if blocked:
            return {
                "available": [],
                "taken": [],
    ...
    def get_admin_availability_matrix(self, date_start, date_end, office_ids=None):
        ...
        if office_ids:
            offices = self.env["office.space"].sudo().browse(office_ids)
        else:
            offices = self.env["office.space"].sudo().search([("active", "=", True)], order="name asc")
    ...
    def _get_admin_email(self):
        ...
        email = param.get_param("office_booking.admin_email")
        if not email:
