...
class OfficeWebsite(http.Controller):
    ...
    def offices_list(self, **kwargs):
        ...
        return request.render("office_booking.website_office_list", {"offices": offices})

    @http.route(
        "/offices/<model('office.space'):office>",
        type="http",
        auth="user",
        website=True,
        methods=["GET", "POST"],
    ...
    def office_detail(self, office, **post):
        ...
        if request.httprequest.method == "POST":
            ...
            if discount_code_raw:
                discount_rec = request.env["office.discount"].sudo().search(
            ...
            if payment_mode == "plan":
                ...
                return request.render("office_booking.website_booking_thanks", {
                    "booking": booking,
    ...
    def validate_discount_code(self, code=None, office_id=None, slot_type=None, **kw):
        ...
        discount = request.env["office.discount"].sudo().search(
        ...
        if office_id and slot_type:
            office = request.env["office.space"].sudo().browse(int(office_id))
    ...
    def office_slot_availability(self, office_id=None, day=None, exclude_id=None, **kw):
        ...
        return request.env["office.booking"].sudo().get_availability(
...
class OfficePortal(CustomerPortal):
    ...
    def _find_bold_record(self, order_id):
        ...
        if order_id.startswith("SPPOT-BOOK-"):
            rec = request.env["office.booking"].sudo().search(
                [("bold_order_id", "=", order_id)], limit=1
            )
            return (rec or None), "booking"

        if order_id.startswith("SPPOT-PLAN-"):
            rec = request.env["office.subscription"].sudo().search(
                [("bold_order_id", "=", order_id)], limit=1
            )
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "booking_count" in counters:
            values["booking_count"] = request.env["office.booking"].sudo().search_count(
    ...
    def portal_my_bookings(self, page=1, filterby="all", **kw):
        ...
        return request.render("office_booking.portal_my_bookings", values)
    ...
    def portal_booking_detail(self, booking_id, cancel_error=None, cancel_msg=None, bold_ok=None, **kw):
        booking = request.env["office.booking"].sudo().browse(booking_id).exists()
    ...
    def portal_booking_cancel(self, booking_id, **kw):
        ...
        if booking.payment_mode == "plan":
            ...
            booking._notify_customer("office_booking.mail_template_booking_cancelled_user")
            booking._notify_admin("office_booking.mail_template_booking_cancelled_admin")
    ...
    def portal_booking_receipt(self, booking_id, **kw):
        ...
        pdf_content, _ = request.env['ir.actions.report'].sudo()._render_qweb_pdf(
            'office_booking.action_report_booking_receipt',
            res_ids=[booking.id],
        )
    ...
    def coworking_plans(self, **kwargs):
        plans = request.env["office.plan"].sudo().search([("active", "=", True)])
        partner = request.env.user.partner_id
        active_subscription = request.env["office.subscription"].sudo().search([
            ("partner_id", "=", partner.id),
            ("state", "=", "active"),
        ], limit=1) or False
        return request.render("office_booking.coworking_plans_page", {
            "plans": plans,
    ...
    def coworking_checkout(self, plan_id, **kwargs):
        plan = request.env["office.plan"].sudo().browse(plan_id)
        ...
        if active_subscription:
            ...
            return request.render("office_booking.coworking_checkout_page", {
                "plan": plan,
                "active_subscription": active_subscription,
            })
        ...
        Subscription = request.env["office.subscription"].sudo()
        ...
        return request.render("office_booking.coworking_checkout_page", values)
    ...
    def my_coworking_dashboard(self, plan_ok=None, **kwargs):
        ...
        all_bookings = request.env['office.booking'].sudo().search([
            ('partner_id', '=', partner.id)
        ], order="date desc")

        # Para la tabla del historial: las más recientes (por fecha de creación)
        bookings = request.env['office.booking'].sudo().search([
            ('partner_id', '=', partner.id)
        ...
        past_subscriptions = request.env['office.subscription'].sudo().search([
            ('partner_id', '=', partner.id),
        ...
        plan_bookings = False
        if subscription:
            plan_bookings = request.env['office.booking'].sudo().search([
                ('partner_id', '=', partner.id),
                ('payment_mode', '=', 'plan'),
                ('subscription_id', '=', subscription.id),
            ], order="date desc")
...
class OfficeExportController(http.Controller):
    ...
    def export_bookings(self, date_from=None, date_to=None, state=None, **kw):
        ...
        bookings = env['office.booking'].sudo().search(
    ...
    def export_occupancy(self, date_from=None, date_to=None, office_id=None, **kw):
        ...
        if office_id:
            try:
                ...
                pass
        offices = env['office.space'].sudo().search(office_domain, order='name')
        ...
        bookings = env['office.booking'].sudo().search(domain)
